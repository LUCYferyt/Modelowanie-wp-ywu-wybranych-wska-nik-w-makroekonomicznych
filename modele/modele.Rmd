...
polish_term <- function(term) {
  term <- trimws(term)
  term <- sub("^[A-Za-z0-9_.]+::", "", term)  # usuwa np. plm::lag(...)
  ...
  if (grepl("^lag\\(", term)) {
    ...
    v <- trimws(parts[1])
    k <- suppressWarnings(as.integer(trimws(parts[2])))

    # jeśli w lag siedzi ln_..., zdejmij ln_
    v_base <- if (grepl("^ln_", v)) sub("^ln_", "", v) else v

    # specjalne etykiety dla Twoich zmiennych zależnych
    if (v_base == "arrivals") {
      lab <- "Przyjazdy turystów"
    } else if (v_base == "receipts") {
      lab <- "Wpływy z turystyki"
    } else {
      lab <- label_var(v_base)
    }

    if (!is.na(k)) return(paste0("Opóźnione ", lab, " (t-", k, ")"))
    return(paste0("Opóźnione ", lab))
  }

  if (grepl("^lag_d_", term)) {
    base <- sub("^lag_d_", "", term)
    base <- sub("^ln_", "", base)
    if (base == "arrivals") return("Opóźniona dynamika przyjazdów turystów")
    if (base == "receipts") return("Opóźniona dynamika wpływów z turystyki")
    return(paste0("Opóźniona dynamika ", label_var(base)))
  }

  if (grepl("^d_ln_", term)) {
    base <- sub("^d_ln_", "", term)
    if (base == "arrivals") return("Dynamika przyjazdów turystów")
    if (base == "receipts") return("Dynamika wpływów z turystyki")
    return(paste0("Dynamika ", label_var(base)))
  }

  if (grepl("^ln_", term)) {
    base <- sub("^ln_", "", term)
    if (base == "arrivals") return("Przyjazdy turystów (log)")
    if (base == "receipts") return("Wpływy z turystyki (log)")
    return(paste0(label_var(base), " (log)"))
  }
...
polonizuj_tabele <- function(df) {
  if ("term" %in% names(df)) {
    df <- df %>% dplyr::mutate(term = sapply(term, polish_term))
  }
  if ("Zmienna" %in% names(df)) {
    df <- df %>% dplyr::mutate(Zmienna = sapply(Zmienna, polish_term))
  }
...
dep_var_pl <- c(
  "ln_arrivals" = "Przyjazdy turystów (log)",
  "ln_receipts" = "Wpływy z turystyki (log)"
)
...
fe_panel_groups_key <- dplyr::bind_rows(
  ...
  mutate(
    dep_var = dplyr::recode(dep_var, !!!dep_var_pl, .default = dep_var),
    dep_group = dplyr::recode(dep_group, !!!dep_group_pl, .default = dep_group)
  ) %>%
  polonizuj_tabele() %>%                           # term/estimate/std.error/statistic/p.value -> PL
  dplyr::rename(
    `Grupa krajów` = dep_group,
    `Zmienna zależna` = dep_var
  ) %>%
  dplyr::select(`Grupa krajów`, `Zmienna zależna`, Zmienna,
                Współczynnik, `Błąd standardowy`, Statystyka, `Wartość p`)
...
ols_fit_stats <- dplyr::bind_rows(
  ...
  dplyr::mutate(
    dep_var = dplyr::recode(dep_var, !!!dep_var_pl)
  ) %>%
  dplyr::rename(
    `Zmienna zależna` = dep_var,
    `Liczba obserwacji` = nobs,
    `R²` = r.squared,
    `Skorygowane R²` = adj.r.squared,
    `Sigma (błąd reszt)` = sigma,
    `AIC` = AIC,
    `BIC` = BIC
  )
...
if (length(ts_models_arrivals) > 0) {
  ts_fit_stats <- purrr::imap_dfr(
    ...
    dplyr::mutate(
      kraj = dplyr::recode(kraj, !!!country_pl, .default = kraj)
    ) %>%
    dplyr::arrange(AIC) %>%
    dplyr::rename(
      `Kraj` = kraj,
